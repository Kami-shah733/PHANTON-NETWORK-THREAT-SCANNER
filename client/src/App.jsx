...
export default function App() {
  ...
  const handleScan = async (ip) => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/scan", { target_ip: ip });
      ...
      <div style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)", padding: "28px 24px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "20px" }}>
          <div>
            ...
            <p className="p-hero-sub">Performs TCP port scans on target hosts, detects running services, and evaluates security exposure using rule-based risk analysis. Built on Nmap 7.99.</p>
          </div>
      ...
      <div className="p-main">
        ...
        <div className="p-content">
          ...
          <div className="p-sec-hdr">
            <div className="p-sec-title"><div className="p-sec-bar"></div>Scan Output</div>
          ...
          {scanResult && !loading && (
            <div className="p-result">
              ...
              <div className="p-score-row">
                ...
                <div className="p-score-track"><div className="p-score-fill" style={{ width: `${scanResult.risk_score}%` }}></div></div>
          ...
          {!scanResult && !loading && (
            <div className="p-empty">[ No scan data ]<br/>Enter a target IP and run a scan to begin.</div>
          )}

          <div className="p-sec-hdr">
            <div className="p-sec-title"><div className="p-sec-bar"></div>Port Analysis</div>
            <span className="p-sec-badge">
              {scanResult ? `${scanResult.ports.filter(p=>p.status==="open").length} open / ${scanResult.ports.length} scanned` : "—"}
            </span>
          </div>
        ...
        <div className="p-sidebar">
          ...
          <div style={{ borderBottom: "1px solid var(--border)" }}>
            ...
            <div className="p-meter-item">
              <div className="p-meter-hdr"><span style={{color:"var(--red)"}}>High Risk</span><span style={{color:"var(--dim)"}}>{history.filter(h=>h.risk_level==="High").length} hosts</span></div>
              <div className="p-meter-track"><div className="p-meter-fill" style={{width:"20%",background:"var(--red)"}}></div></div>
            </div>
            <div className="p-meter-item">
              <div className="p-meter-hdr"><span style={{color:"var(--yellow)"}}>Medium Risk</span><span style={{color:"var(--dim)"}}>{history.filter(h=>h.risk_level==="Medium").length} hosts</span></div>
              <div className="p-meter-track"><div className="p-meter-fill" style={{width:"40%",background:"var(--yellow)"}}></div></div>
            </div>
            <div className="p-meter-item">
              <div className="p-meter-hdr"><span style={{color:"var(--green)"}}>Low Risk</span><span style={{color:"var(--dim)"}}>{history.filter(h=>h.risk_level==="Low").length} hosts</span></div>
              <div className="p-meter-track"><div className="p-meter-fill" style={{width:"80%",background:"var(--green)"}}></div></div>
            </div>
          </div>
